...
// (no la voz robótica del dispositivo). Texto corto (preguntas) — límite ~200 caracteres.
export default async function handler(req, res) {
  try {
    const q = String((req.query && req.query.q) || '').slice(0, 200)
    const tl = String((req.query && req.query.tl) || 'es')
