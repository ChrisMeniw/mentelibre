...
// Branding fijo en TODA pantalla, sin scroll: logo + "Impulsado por Chris Meniw Foundation".
export default function FoundationBadge() {
  const { t } = useLang()
  return (
    <div
      className="fixed bottom-0 inset-x-0 z-30 pointer-events-none safe-bottom"
      aria-label={`${t('poweredBy')} Chris Meniw Foundation`}
    >
      <div
      ...
      >
        ...
        <span>
          {t('poweredBy')} <span className="text-[var(--violet-light)]">Chris Meniw Foundation</span>
        </span>
      </div>
    </div>
  )
